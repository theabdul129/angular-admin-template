...
@Component({
  selector: 'search-business-account',
  templateUrl: './business-account.component.html',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => SearchBusinessAccountComponent),
      multi: true
    }
  ]
})
export class SearchBusinessAccountComponent implements ControlValueAccessor {
  ...
  getSearch() {
    this.searching = true;
    let data = {
      "accountNumber": this.searchText,
      "page": 0,
      "pageSize": 100,
    }
    this.searchService.searchBusinessAccounts(data).subscribe(data => {
      this.ngZone.run(() => {
